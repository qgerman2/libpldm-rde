...
// Custom Headers
#define PLDM_RDE_EXPAND_TYPE "PLDM-RDE-Expand-Type"
#define EXPAND_DOT	     "."
#define EXPAND_TILDE	     "~"
#define EXPAND_STAR	     "*"
...
enum pldm_rde_commands {
	...
	PLDM_SUPPLY_CUSTOM_REQUEST_PARAMETERS = 0x11,
 ...
 */
struct pldm_supply_custom_request_parameters_req {
	uint32_t resource_id;
	uint16_t operation_id;
 ...
 */
struct pldm_supply_custom_request_parameters_resp {
	uint8_t completion_code;
	uint8_t operation_status;
	uint8_t completion_percentage;
	uint32_t completion_time_seconds;
 ...
 * @param[in] resource_id - The ResourceID.
 * @param[in] operation_id - Identification number for this operation.
 * @param[in] link_expand - The value of a $levels qualifier to a $expand
 * query option.
 * @param[in] collection_skip - The value of a $skip query option.
 * @param[in] collection_top - The value of a $top query option.
 * @param[in] pagination_offset - The page offset for paginated response data.
 * @param[out] msg - Request message will be written to this.
 * @return pldm_completion_codes.
 */
...
/**
 * @brief Encode SupplyCustomRequestParameters request's etags.
 *
 * @param[in] etag_operation -  ETagOperation type.
 * @param[in] etag_count -  Number of ETags supplied in this message;
 * should be zero if ETagOperation above is ETAG_IGNORE and nonzero otherwise.
 * @param[in] etag_formats -  The list of etag formats.
 * @param[in] etags -  The list of etags.
 * @param[in] end_of_etags_offset - Offset in msg to encode etags.
 * @param[out] msg - Request message will be written to this.
 * @return pldm_completion_codes.
 */
...
/**
 * @brief Encode SupplyCustomRequestParameters request's headers.
 *
 * @param[in] Header_count -  The number of RDE custom headers being supplied
 * in this operation.
 * @param[in] hdrname_formats - List if hdrnames formats.
 * @param[in] hdrnames - List if custom headers, including the PLDM-RDE-
 * prefix.
 * @param[in] hdrparam_formats - List if hdrparams formats.
 * @param[in] hdrparams -  List of parameters associated with
 * the header.
 * @param[in] header_offset - Offset in msg to encode headers.
 * @param[out] msg - Request message will be written to this.
 * @return pldm_completion_codes.
 */
...
/**
 * @brief Encode SupplyCustomRequestParameters response.
 *
 * @param[in] instance_id - Message's instance id.
 * @param[in] completion_code - PLDM completion code.
 * @param[in] operation_status - Status of the operation.
...
int encode_supply_custom_request_parameters_resp(
	...
	uint32_t result_transfer_handle,
	const union pldm_rde_permission_flags *permission_flags,
	uint32_t response_payload_length, enum pldm_rde_varstring_format etag_format, char *etag,
	const uint8_t *response_payload, struct pldm_msg *msg);

/**
 * @brief Decode SupplyCustomRequestParameters Resp
 *
 * @param[in] msg - Response message
 * @param[in] payload_length - Expected length of the response, since the
 * response could be equal to the negotiated transfer chunk size, the requester
...
int decode_supply_custom_request_parameters_resp(
	...
	uint8_t *completion_code, uint8_t *completion_percentage,
	uint8_t *operation_status, uint32_t *completion_time_seconds,
	uint32_t *result_transfer_handle, uint32_t *response_payload_length,
	union pldm_rde_permission_flags **permission_flags,
	union pldm_rde_op_execution_flags **operation_execution_flags,
	struct pldm_rde_varstring **resp_etag, uint8_t **response_payload);
